...
for dof in range(1, 8):
    mdhs = sample_mdh(NUM_ROB * 2, dof=dof).to(torch.float32).to(device)
    ...
    for mdh in pbar:
        ...
        data = {
            "mdh": pyarrow.FixedShapeTensorArray.from_numpy_ndarray(
                mdh.flatten().unsqueeze(0).expand(labels.shape[0], -1).cpu().numpy()
            ),
            "poses": pyarrow.FixedShapeTensorArray.from_numpy_ndarray(poses.cpu().numpy()),
            "labels": pyarrow.array(labels.cpu().numpy()),
            "joints": pyarrow.FixedShapeTensorArray.from_numpy_ndarray(joints.cpu().numpy()),
        } # balanced batches already with weights, lower precision,
